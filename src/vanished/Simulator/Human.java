...
public class Human {
	...
	private void BBB(ArrayList<ArrayList<TryResult>> resultsReal, ArrayList<ArrayList<TryResult>> resultsVirtual) throws Exception {
		...
		HashMap<Room, ArrayList<ArrayList<TryResult>>> room2ResultSequence = new HashMap<Room, ArrayList<ArrayList<TryResult>>>();
		{
			for (ArrayList<TryResult> resultSequence : resultsVirtual) {
				...
				ArrayList<ArrayList<TryResult>> results = room2ResultSequence.get(roomVirtual);
				if (results == null) {
					results = new ArrayList<ArrayList<TryResult>>();
					room2ResultSequence.put(roomVirtual, results);
				}
		...
		for (Entry<Room, ArrayList<ArrayList<TryResult>>> e : room2ResultSequence.entrySet()) {
	...
	private void EvaluateAndExecuteAction(ArrayList<ArrayList<TryResult>> results, Room roomVirtual) throws Exception {
		...
		double weightTotal = 0;
		{
			...
			double rate = 1;
			for (int i = 0; i < numSequence; i++) {
				double weight = Math.exp(scores[i] / scoreMax * rate);
				weights[i] = weight;
				weightTotal += weight;
			}
		}
